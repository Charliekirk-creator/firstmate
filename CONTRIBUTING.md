...
A GitHub Actions check (`Require no-mistakes`) runs on PRs targeting `main` and requires both the deterministic signature and a parseable structured attestation from no-mistakes v1.60.2 or newer.
That version is the floor because it rebinds the existing attestation after each pipeline-owned CI repair push instead of leaving a valid run bound to its prior head.
...
3. Initialize the gate with your fork as the push target: `no-mistakes init --fork-url git@github.com:<you>/firstmate.git` (contributing to firstmate requires **no-mistakes v1.60.2+** for settled structured attestation across CI repairs; without a fork, plain `no-mistakes init` still works for maintainers with push access).
