...
A released-version record that predates embedded origin and key fields remains compatible only when the origin's existing reviewed metadata inventory uniquely binds that composed hold identity; an absent or ambiguous inventory cannot supply legacy identity.
