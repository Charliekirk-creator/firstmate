...
A released-version record that predates embedded origin and key fields remains compatible when its composed hold identity has exactly one valid origin/key decomposition, when an existing exact attestation matches the complete record, or when its exact reviewed owner metadata still claims the key and every alternate decomposition has reviewed metadata that excludes the colliding key; that migration atomically attests the complete record before ephemeral metadata can disappear.
Ambiguous legacy identities are never rebound from claimant metadata unless every alternate decomposition is explicitly disproven, and absent, conflicting, foreign, or changed proof cannot supply identity.
