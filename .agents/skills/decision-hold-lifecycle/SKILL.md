...
Completion metadata must preserve whether each key was verified as current or supplied as historical, so an exact retry may recognize a current key that has since resolved without letting older history substitute for a missing active generation; a nonempty released-version inventory without that provenance must be reclassified explicitly through `complete`, never inferred from whichever record remains.
