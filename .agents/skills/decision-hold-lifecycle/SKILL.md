...
A released-version record that predates embedded origin and key fields remains compatible only when its composed hold identity has one unambiguous origin/key decomposition and live reviewed metadata in the authoritative home persists an exact durable attestation before teardown, or when an existing attestation still matches that record; absent, ambiguous, foreign, or changed proof cannot supply legacy identity.
