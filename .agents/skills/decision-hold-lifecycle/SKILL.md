...
A released-version record that predates embedded origin and key fields remains compatible only when an existing exact attestation matches that record, or when live reviewed metadata is the sole durable owner among every valid decomposition of its composed hold identity in the authoritative home and persists an exact attestation before teardown; absent, conflicting, foreign, or changed proof cannot supply legacy identity.
