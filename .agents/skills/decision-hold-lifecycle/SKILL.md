...
A released-version record that predates embedded origin and key fields remains compatible when its composed hold identity has exactly one valid origin/key decomposition or when an existing exact attestation matches the complete record; ambiguous legacy identities are never rebound from claimant metadata, and absent, conflicting, foreign, or changed proof cannot supply identity.
