...
Retention-affecting task completion must use `bin/fm-decision-hold.sh task-done`, and explicit bounded pruning must use its `retention-prune` command, so the exact records archived by normal tasks-axi retention receive transition provenance.
...
6. If the captain authorizes dependent work, record it with normal tasks-axi commands and block it by the hold identity; use the script's `task-done` command for task completion that may advance Done retention.
