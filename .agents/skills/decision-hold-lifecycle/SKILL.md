...
A released-version record that predates embedded origin and key fields remains compatible only when live reviewed metadata uniquely binds that composed hold identity and persists an exact durable attestation before teardown, or when an existing attestation still matches that record; absent, ambiguous, or changed proof cannot supply legacy identity.
