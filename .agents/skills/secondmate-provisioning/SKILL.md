...
Before either placement moves a linked or legacy-unlinked backlog row, the helper uses `fm-work-identity.sh` to freeze source intake and durably prepare an exact transfer bound to the destination path and stable `secondmate:<id>` home identity; a failed pre-move local batch cancels its prepares without publishing destination sidecars, while an uncertain remote prepare remains fail-closed for recovery.
...
For a remote route, the same helper then moves the dependency-closed set atomically from the main backlog into `data/handoff/<id>.outbox.md`, transfers that backlog-format outbox through `fm-on.sh`, and lets the remote home's `fm-backlog-receive.sh` move every not-already-present key under the destination lock.
After a new local placement or a remote outbox receipt and identity commit become durable, the helper sends one marked routed-work instruction through the receiving secondmate's recorded endpoint; missing or failed delivery makes the command fail loudly with the moved work intact, and the same handoff command retries known-undelivered wake intent without moving an already-present item again.
An unresolved delivery attempt is never blindly resent.
For a remote route, the outbox remains until backlog receipt, destination identity commit, source tombstones, and receiver wake are confirmed; `--resume-pending` revalidates retained per-task identity prepare records and retries unfinished work, while the script header owns its stable wake-correlation recovery state.
The identity owner's prepare/commit state adds no tasks-axi release beyond the already-required atomic `mv` capability and is not a second backlog journal.
