...
The producer maps only the state enum from Firstmate's bounded local current-state reader and discards its detail prose; remote-worker state stays unavailable so the default command never opens a connection.
The current Firstmate records do not authoritatively publish worker liveness proof, context duration, typed plan and stage hierarchy, commands, delivery or acceptance relations, or retained decision history for this interface, so those surfaces remain partial or unavailable instead of being reconstructed.
