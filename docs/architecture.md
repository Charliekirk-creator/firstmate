...
Before hashing Pi or pi-signed captures, the watcher normalizes only the proven dynamic `HH:MM` field on the structured context footer immediately above Pi's ready or working state line, so clock-only rerenders remain unchanged while all other pane content and other harnesses remain significant.
A version sidecar and durable migration journal convert existing Pi markers without a migration-only wake: a stale classification proven equivalent apart from that clock is carried with its wedge state, while an unproven content change starts a fresh baseline so genuine progress can surface, and an interrupted conversion resumes before further classification.
