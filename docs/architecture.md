...
The Workstack Compass executable model under the explicitly supplied application root remains the sole snapshot-schema owner: its `snapshot_from_evidence` adapter constructs the candidate from Firstmate's sanitized evidence envelope, and its validator checks that complete candidate before publication. Both run through a bounded subprocess with a sanitized environment, no network or write authority, and a copy-isolated validation candidate.
