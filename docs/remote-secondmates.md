...
- The required remote tool set is `git`, `jq`, `python3`, Herdr 0.7.5 or newer (protocol 17), compatible `tasks-axi`, `treehouse`, and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, or `grok`. Kimi is excluded because Herdr does not expose a transaction-scoped prompt receipt for deterministic instruction delivery.
