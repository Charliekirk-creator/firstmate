...
- The required remote tool set is `git`, `jq`, `python3`, Herdr protocol 14 or newer, compatible `tasks-axi`, `treehouse`, and at least one of `claude`, `codex`, `opencode`, `pi`, `pi-signed`, or `grok`. Kimi is excluded because Herdr does not expose a transaction-scoped prompt receipt for deterministic instruction delivery.
