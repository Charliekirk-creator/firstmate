...
`fm-spawn.sh` accepts `tmux`, `herdr`, `zellij`, `orca`, and `cmux` for ship and scout tasks; `backend=orca` and `backend=cmux` both still refuse `--secondmate` until secondmate launch semantics are designed for each. Persistent Kimi secondmates require `backend=tmux`, whose queued text-and-receipt operation can distinguish unsent input from accepted input after interruption; non-tmux Kimi ship and scout tasks remain supported.
