...
`fm-brief.sh` embeds the canonical payload and digest in the generated instructions, and `fm-spawn.sh` refuses a mismatch before creating a worker endpoint.
...
Tasks without a record remain compatible and appear explicitly as unlinked.
