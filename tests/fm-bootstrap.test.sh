...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.60.2 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
minimum no-mistakes version is accepted^no-mistakes version v1.60.2 (fake)^empty
newer no-mistakes patch is accepted^no-mistakes version v1.60.3 (fake)^empty
newer no-mistakes minor is accepted^no-mistakes version v1.61.0 (fake)^empty
newer no-mistakes major is accepted^no-mistakes version v2.0.0 (fake)^empty
pre-settlement no-mistakes patch reports an upgrade^no-mistakes version v1.60.1 (fake)^missing
legacy no-mistakes attestation build reports an upgrade^no-mistakes version v1.48.0 (fake)^missing
